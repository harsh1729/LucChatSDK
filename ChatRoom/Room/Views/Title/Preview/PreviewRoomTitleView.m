...
- (void)awakeFromNib
{
    ...
    [self.leftButton setTitle:NSLocalizedStringFromTableInBundle(@"decline", @"Vector",[NSBundle bundleForClass:[self class]], nil) forState:UIControlStateNormal];
    [self.leftButton setTitle:NSLocalizedStringFromTableInBundle(@"decline", @"Vector",[NSBundle bundleForClass:[self class]], nil) forState:UIControlStateHighlighted];
    ...
    [self.rightButton setTitle:NSLocalizedStringFromTableInBundle(@"join", @"Vector",[NSBundle bundleForClass:[self class]], nil) forState:UIControlStateNormal];
    [self.rightButton setTitle:NSLocalizedStringFromTableInBundle(@"join", @"Vector",[NSBundle bundleForClass:[self class]], nil) forState:UIControlStateHighlighted];
...
- (void)refreshDisplay
{
    ...
    if (self.roomPreviewData)
    {
            ...
            [self.roomAvatar setImageURI:self.roomAvatarURL
                     ...
                     andImageOrientation:UIImageOrientationUp
                           toFitViewSize:self.roomAvatar.frame.size
                            ...
                            previewImage:[MXKTools paintImage:[UIImage imageNamed:@"placeholder" inBundle:[NSBundle bundleForClass:[self class]] compatibleWithTraitCollection:nil]
        ...
        {
            self.roomMembers.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"room_title_members", @"Vector",[NSBundle bundleForClass:[self class]], nil), @(self.roomPreviewData.numJoinedMembers)];
        }
        else if (self.roomPreviewData.numJoinedMembers == 1)
        {
            self.roomMembers.text = NSLocalizedStringFromTableInBundle(@"room_title_one_member", @"Vector",[NSBundle bundleForClass:[self class]], nil);
        }
        else
        {
        ...
        if (self.roomPreviewData.roomDataSource)
        {
            ...
            self.subNoticeLabel.text = NSLocalizedStringFromTableInBundle(@"room_preview_subtitle", @"Vector",[NSBundle bundleForClass:[self class]], nil);
        }
        else
        {
            self.subNoticeLabel.text = nil;
        ...
        if (self.roomPreviewData.emailInvitation.email)
        {
            ...
            self.previewLabel.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"room_preview_invitation_format", @"Vector",[NSBundle bundleForClass:[self class]], nil), self.roomPreviewData.emailInvitation.inviterName];
            
            // Warn the user that the email is not bound to his matrix account
            self.subNoticeLabel.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"room_preview_unlinked_email_warning", @"Vector",[NSBundle bundleForClass:[self class]], nil), self.roomPreviewData.emailInvitation.email];
        }
        else
        {
            ...
            if (!roomName)
            {
                roomName = NSLocalizedStringFromTableInBundle(@"room_preview_try_join_an_unknown_room_default", @"Vector",[NSBundle bundleForClass:[self class]], nil);
            }
            ...
            self.previewLabel.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"room_preview_try_join_an_unknown_room", @"Vector",[NSBundle bundleForClass:[self class]], nil), roomName];
        }
    }
    else if (self.mxRoom)
    {
        ...
        void (^onRoomMembers)(MXRoomMembers *roomMembers, BOOL allMembers) = ^void(MXRoomMembers *roomMembers, BOOL allMembers)
        {
            ...
            self.previewLabel.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"room_preview_invitation_format", @"Vector",[NSBundle bundleForClass:[self class]], nil), inviter];
        };
