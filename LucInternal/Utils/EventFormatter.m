...
- (NSAttributedString *)attributedStringFromEvent:(MXEvent *)event withRoomState:(MXRoomState *)roomState error:(MXKEventFormatterError *)error
{
            ...
            || [event.type isEqualToString:kWidgetModularEventTypeString]))
    {
        ...
        if (widget)
        {
            ...
            if (widget.isActive)
            {
                    displayText = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"event_formatter_widget_added", @"Vector",[NSBundle bundleForClass:[self class]], nil),
                ...
                for (MXEvent *widgetStateEvent in widgetStateEvents)
                {
                    if ([widgetStateEvent.stateKey isEqualToString:widget.widgetId])
                    {
                        ...
                        if (activeWidget.isActive)
                        {
                                ...
                                displayText = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"event_formatter_widget_removed", @"Vector",[NSBundle bundleForClass:[self class]], nil),
        ...
        && [event.decryptionError.domain isEqualToString:MXDecryptingErrorDomain])
    {
        ...
        if (event.decryptionError.code == MXDecryptingErrorUnknownInboundSessionIdCode)
        {
            ...
            [attributedStringWithRerequestMessage appendAttributedString:
             [[NSAttributedString alloc] initWithString:NSLocalizedStringFromTableInBundle(@"event_formatter_rerequest_keys_part1_link", @"Vector",[NSBundle bundleForClass:[self class]], nil)
                                             attributes:@{
            ...
            [attributedStringWithRerequestMessage appendAttributedString:
             [[NSAttributedString alloc] initWithString:NSLocalizedStringFromTableInBundle(@"event_formatter_rerequest_keys_part2", @"Vector",[NSBundle bundleForClass:[self class]], nil)
                                             attributes:@{
                                                          NSForegroundColorAttributeName: self.sendingTextColor,
                                                          NSFontAttributeName: self.encryptedMessagesTextFont
                                                          }]];
    ...
    else if (self.showEditionMention && event.contentHasBeenEdited)
    {
        ...
        [attributedStringWithEditMention appendAttributedString:
         [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %@", NSLocalizedStringFromTableInBundle(@"event_formatter_message_edited_mention", @"Vector",[NSBundle bundleForClass:[self class]], nil)]
                                         attributes:@{
                                                      NSLinkAttributeName: linkActionString,
...
- (NSAttributedString*)attributedStringFromEvents:(NSArray<MXEvent*>*)events withRoomState:(MXRoomState*)roomState error:(MXKEventFormatterError*)error
{
    ...
    if (events.count)
    {
        if (events[0].eventType == MXEventTypeRoomMember)
        {
            ...
            displayText = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"event_formatter_member_updates", @"Vector",[NSBundle bundleForClass:[self class]], nil), events.count];
        }
    }
...
- (NSString*)dateStringFromDate:(NSDate *)date withTime:(BOOL)time
{
    ...
    else if (interval > 0)
    {
        if (time)
        {
            ...
            return [NSString stringWithFormat:@"%@ %@", NSLocalizedStringFromTableInBundle(@"yesterday", @"Vector",[NSBundle bundleForClass:[self class]], nil), [super dateStringFromDate:date withTime:YES]];
        }
        return NSLocalizedStringFromTableInBundle(@"yesterday", @"Vector",[NSBundle bundleForClass:[self class]], nil);
    }
    else if (interval > - 60*60*24)
    {
        if (time)
        ...
        return NSLocalizedStringFromTableInBundle(@"today", @"Vector",[NSBundle bundleForClass:[self class]], nil);
    }
    else
    {
        // Date in future
...
- (NSAttributedString*)roomCreatePredecessorAttributedStringWithPredecessorRoomId:(NSString*)predecessorRoomId
{
    ...
    NSString *roomPredecessorReasonString = [NSString stringWithFormat:@"%@\n", NSLocalizedStringFromTableInBundle(@"room_predecessor_information", @"Vector",[NSBundle bundleForClass:[self class]], nil)];
    ...
    NSString *predecessorRoomLinkString = NSLocalizedStringFromTableInBundle(@"room_predecessor_link", @"Vector",[NSBundle bundleForClass:[self class]], nil);
