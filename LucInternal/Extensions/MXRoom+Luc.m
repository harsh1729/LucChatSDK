...
- (void)setRoomTag:(NSString*)tag completion:(void (^)(void))completion
{
    ...
    NSLog(@"[MXRoom+Luc] Update the room %@ tag from %@ to %@ with tag order %@", self.roomId, oldTag, tag, tagOrder);
    ...
    [self replaceTag:oldTag
           ...
           withOrder:tagOrder
             success: ^{
                 ...
                 NSLog(@"[MXRoom+Luc] Failed to update the tag %@ of room (%@)", tag, self.roomId);
...
- (void)mute:(void (^)(void))completion
{
    ...
    else
    {
        ...
        {
            NSLog(@"[MXRoom+Luc] Request in progress: ignore push rule update");
            if (completion)
            {
                completion();
            }
